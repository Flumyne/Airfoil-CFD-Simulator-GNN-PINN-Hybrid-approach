...
def create_geo_file(nozzle_file, output_geo,l_convergent, lc_tuyere=0.0015, lc_col=0.001, lc_domain=0.5, l_domain=2.0, r_exit=4.0):
    """
    ...
    with open(output_geo, 'w') as f:
        ...
        f.write(f"Point(1002) = {{l_domain, 0.0, 0.0, lc_domain}};\n")
        f.write(f"Point(1003) = {{l_outlet, 0.0, 0.0, lc_domain}};\n")    
        f.write(f"Point(1004) = {{l_outlet, r_exit, 0.0, lc_domain}};\n\n")
        ...
        f.write("Line(5) = {1003, 1004};\n")
        f.write(f"Line(6) = {{1004, {len(points)}}};\n\n")
